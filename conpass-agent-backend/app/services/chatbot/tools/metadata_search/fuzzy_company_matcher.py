...
# Matching configuration (環境変数で動的調整可能)
DEFAULT_THRESHOLD = int(os.getenv("FUZZY_MATCH_THRESHOLD", "75"))
...
# Firestoreから読み込む正規化辞書のキャッシュ
_normalization_dict: Optional[Dict[str, str]] = None
_normalization_dict_loaded: bool = False
...
async def load_normalization_dict() -> Dict[str, str]:
    """
    Firestoreから会社名・人名の正規化辞書を読み込む。

    Firestoreの normalization_dict コレクションに以下の形式でドキュメントを格納:
    - company_names: { "表記揺れ": "正式名称", ... }
    - person_names: { "表記揺れ": "正式名称", ... }

    Returns:
        正規化辞書（表記揺れ -> 正式名称のマッピング）
    """
    global _normalization_dict, _normalization_dict_loaded

    if _normalization_dict_loaded:
        return _normalization_dict or {}

    _normalization_dict_loaded = True
    ...
    try:
        ...
        for doc_name in ("company_names", "person_names"):
            doc = await db.collection("normalization_dict").document(doc_name).get()
            if doc.exists:
                data = doc.to_dict() or {}
                combined.update(data)
                logger.info(
                    f"Loaded {len(data)} entries from normalization_dict/{doc_name}"
                )

        _normalization_dict = combined
        logger.info(f"Total normalization dict entries: {len(combined)}")
    except Exception as e:
        logger.warning(f"Failed to load normalization dict from Firestore: {e}")
        _normalization_dict = {}

    return _normalization_dict


def apply_normalization_dict(name: str) -> str:
    """
    正規化辞書を使って名前を正規化する（同期版）。
    辞書が未ロードの場合はそのまま返す。
    ...
    Returns:
        正規化後の名前（辞書にマッチしなければ元の名前）
    """
    if _normalization_dict and name in _normalization_dict:
        canonical = _normalization_dict[name]
        logger.debug(f"Normalization dict: '{name}' -> '{canonical}'")
        return canonical
    return name
...
def normalize_company_name(company_name: str) -> str:
    ...
    - Strips whitespace
    - Applies normalization dictionary (Firestore)
    ...
    # 正規化辞書の適用（ロード済みの場合）
    normalized = apply_normalization_dict(normalized)
