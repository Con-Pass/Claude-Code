...
def _fix_company_must_should_duplication(filter_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    LLMが should と must 両方に会社名フィールドを出力するバグを修正する。

    会社名のみのクエリで should に正しく会社条件が入っているのに
    must にも同じ会社条件が重複して入っている場合、must から会社条件を除去する。
    """
    if not isinstance(filter_dict, dict):
        return filter_dict

    must = filter_dict.get("must", []) or []
    should = filter_dict.get("should", []) or []

    if not (must and should):
        return filter_dict

    # should が会社名フィールドのみで構成されているか確認
    should_has_company = all(
        isinstance(c, dict) and c.get("key") in _COMPANY_FIELDS
        for c in should
    )
    if not should_has_company:
        return filter_dict

    # must から会社名フィールドの条件を除去
    non_company_must = [
        c for c in must
        if not (isinstance(c, dict) and c.get("key") in _COMPANY_FIELDS)
    ]

    if len(non_company_must) < len(must):
        logger.info(
            f"[filter_fix] must から会社名条件 {len(must) - len(non_company_must)} 件を除去し、"
            f"should に移動済み（正しい構造）"
        )
        result = dict(filter_dict)
        if non_company_must:
            result["must"] = non_company_must
        else:
            result.pop("must", None)
        return result
...
async def _generate_filter(
    ...
    if filter_response.filter:
        try:
            ...
            # Fix LLM bug: company fields duplicated in both must and should
            cleaned_filter_dict = _fix_company_must_should_duplication(cleaned_filter_dict)
