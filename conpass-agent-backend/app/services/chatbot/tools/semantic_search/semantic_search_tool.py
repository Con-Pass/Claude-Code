...
# 法令規制コレクション設定
LAW_QDRANT_COLLECTION = os.getenv("LAW_QDRANT_COLLECTION", settings.QDRANT_LAWS_COLLECTION)
LAW_SEARCH_TOP_K = int(os.getenv("LAW_SEARCH_TOP_K", "10"))
...
async def _search_qdrant_dense_fallback(
    ...
    body: Dict[str, Any] = {
        "query": embedding,
        "using": "dense",  # named vector (collection has no default unnamed vector)
        "limit": max(1, top_k),
...
async def _search_law_regulations(
    dense_embedding: List[float],
    top_k: int,
) -> List[Dict[str, Any]]:
    """
    法令規制コレクション（law_regulations）からDense検索を実行する。

    法令コレクションはdirectory_idフィルタを使わず、全法令を対象に検索する。

    Args:
        dense_embedding: クエリのDenseベクトル
        top_k: 返却件数
    ...
    try:
        response = client.query_points(
            collection_name=LAW_QDRANT_COLLECTION,
            prefetch=[
                models.Prefetch(
                    query=dense_embedding,
                    using="dense",
        ...
        logger.info(
            f"Law regulation search returned {len(results)} results"
        )
        return results

    except Exception as e:
        logger.warning(
            f"Law regulation search failed (collection may not exist yet): {e}"
        )
        return []


async def _fetch_related_contracts(
    contract_ids: List[int],
    directory_ids: List[int],
) -> List[Dict[str, Any]]:
    """
    検索結果に含まれる related_contract メタデータを使って、
    関連契約書をフォロー検索（2段階検索）する。

    Args:
        contract_ids: 元の検索結果から抽出した contract_id のリスト
        directory_ids: アクセス権のある directory_id のリスト
    ...
    try:
        # 元の検索結果の contract_id からペイロードを取得して related_contract を抽出
        for cid in contract_ids:
            ...
            scroll_result = client.scroll(
                ...
                scroll_filter=models.Filter(
                    must=conditions,
                    must_not=q_filter.must_not if q_filter else None,
                ),
                limit=1,
                with_payload=True,
                with_vectors=False,
            ...
            for point in points:
                payload = point.payload or {}
                rel = payload.get("related_contract")
                if rel:
                    # related_contract は単一IDまたはリスト
                    if isinstance(rel, list):
                        related_ids.update(int(r) for r in rel if r)
                    else:
                        related_ids.add(int(rel))

        # 元の検索結果に含まれるIDは除外
        related_ids -= set(contract_ids)

        if not related_ids:
            return []

        logger.info(f"Found {len(related_ids)} related contracts to follow up: {related_ids}")
        ...
        for rid in related_ids:
            ...
            if points:
                ...
                related_contracts.append({
                    "contract_id": rid,
                    "contract_url": f"{settings.CONPASS_FRONTEND_BASE_URL}/contract/{rid}",
                    "title": payload.get("契約書名_title", ""),
        ...
        logger.warning(f"Related contracts follow-up failed: {e}")
        return []
...
async def semantic_search(
    ...
    *,
    ...
    deduplicate_by_contract: bool = True,
    include_law_regulations: bool = False,
    include_related_contracts: bool = True,
) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        include_law_regulations: If True, also search law_regulations collection in parallel
        include_related_contracts: If True, auto-follow related_contract metadata (2-stage search)
    """
    ...
    try:
        ...
        top_k = similarity_top_k or SEARCH_TOP_K
        ...
        if include_law_regulations:
            search_tasks.append(
                _search_law_regulations(dense_embedding, LAW_SEARCH_TOP_K)
            )

        search_results = await asyncio.gather(*search_tasks, return_exceptions=True)

        # 契約書検索結果の処理
        contract_points = search_results[0] if not isinstance(search_results[0], Exception) else []
        if isinstance(search_results[0], Exception):
            logger.error(f"Contract search failed: {search_results[0]}")

        # クロスエンコーダリランキング（有効化時のみ）
        contract_points = await rerank_results(query, contract_points)
        ...
        # 法令検索結果があれば追加（インデックス 1: hybrid=0, law=1）
        if include_law_regulations and len(search_results) > 1:
            law_results = search_results[1] if not isinstance(search_results[1], Exception) else []
            if isinstance(search_results[1], Exception):
                logger.warning(f"Law regulation search failed: {search_results[1]}")
                law_results = []

            if law_results:
                # 検索結果に related_laws フィールドとして追加
                for source in result:
                    source["related_laws"] = law_results

        # 関連契約書のフォロー検索（2段階検索）
        if include_related_contracts and result:
            primary_contract_ids = [
                s["contract_id"] for s in result
                if s.get("contract_id") is not None
            ]
            if primary_contract_ids:
                related = await _fetch_related_contracts(
                    primary_contract_ids, directory_ids
                )
                if related:
                    # 検索結果全体に related_contracts を付与
                    for source in result:
                        source["related_contracts"] = related
