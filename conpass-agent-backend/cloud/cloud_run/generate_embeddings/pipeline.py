...
from chunker import chunk_text, chunk_text_with_metadata, chunk_law_article
...
def generate_datasource(
    ...
    Args:
        ...
        source_type: データソースの種別。"contract"（デフォルト）または"law_regulation"

    Returns:
        Dict with processing results: {nodes, processed, failed, skipped, updated, failures}
    """
    logger.info(f"Generate index for the provided data (source_type={source_type})")

    for doc in documents:
        doc.metadata["private"] = False

    if source_type == "law_regulation":
        result = run_law_regulation_pipeline(documents)
    else:
...
# 法令規制コレクション名
LAW_QDRANT_COLLECTION = os.environ.get("LAW_QDRANT_COLLECTION", "law_regulations")


def run_law_regulation_pipeline(documents: List[Document]) -> Dict[str, Any]:
    """
    法令規制ドキュメント用のEmbeddingパイプライン。

    契約書とは異なり:
    - 条文単位の小さいチャンク（256文字）を使用
    - law_regulationsコレクションに格納
    - 法令固有メタデータ（law_name, article_number等）を付与

    Args:
        documents: 法令条文のDocumentリスト

    Returns:
        処理結果dict
    """
    logger.info(f"Starting law regulation pipeline for {len(documents)} documents")

    embedding_model = get_embedding_model()

    # 法令用のQdrantベクトルストアを取得（law_regulationsコレクション）
    law_vector_store = CustomQdrantVectorStore(collection_name=LAW_QDRANT_COLLECTION)
    ...
    for document in documents:
        try:
            ...
            # 法令条文用チャンキング（256文字上限）
            chunks = chunk_law_article(text, max_tokens=256)

            if not chunks:
                logger.warning(
                    f"法令ドキュメント {document.id_} のチャンクが空です"
                )
                continue
            ...
            for node, embedding in zip(nodes, embeddings):
                if embedding is None:
                    raise ValueError(
                        f"法令ドキュメント {document.id_} のEmbedding生成失敗"
                    )
                node.embedding = embedding

            # Qdrantに格納
            law_vector_store.add(nodes)
            ...
            logger.info(
                f"法令 {law_name} {article_number}: {len(nodes)}チャンクを格納"
            )

        except Exception as e:
            failed_count += 1
            error_msg = f"法令ドキュメント {document.id_} の処理エラー: {e}"
    ...
    logger.info(
        f"Law regulation pipeline completed in {batch_elapsed:.1f}s: "
    ...
    return {
        ...
        "skipped": 0,
        "failed": failed_count,
        "failure_rate": failed_count / max(len(documents), 1),
        "failures": failures,
