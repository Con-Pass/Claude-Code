...
def chunk_law_article(
    ...
    """
    法令の条文テキストを小さなチャンクに分割する。

    法令の条文は契約書より短いため、小さいチャンクサイズ（デフォルト256文字）を使用。
    項・号の境界で優先的に分割し、それでもmax_tokensを超える場合は
    文境界（句点）で分割する。

    Args:
        article_text: 条文テキスト
        max_tokens: 最大チャンクサイズ（文字数）
    ...
    article_text = article_text.replace("\u3000", " ").strip()

    # max_tokens以下ならそのまま返す
    if len(article_text) <= max_tokens:
        return [article_text]

    # 項番号（「2.」「３．」「第2項」）や号番号（「(1)」「（１）」）で分割を試みる
    split_pattern = re.compile(
        r"(?=(?:^|\n)\s*(?:第[0-9０-９一二三四五六七八九十]+項|[0-9０-９]+[.．]|[（(][0-9０-９]+[）)]|[①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳]))"
    )

    segments = split_pattern.split(article_text)
    segments = [s.strip() for s in segments if s.strip()]

    # セグメントをmax_tokens以下に結合
    chunks: List[str] = []
    current_chunk = ""
    ...
    for segment in segments:
        ...
        elif len(current_chunk) + len(segment) + 1 <= max_tokens:
    ...
    # 各チャンクがまだmax_tokensを超えている場合、文境界で再分割
    final_chunks: List[str] = []
    for chunk in chunks:
        if len(chunk) <= max_tokens:
            final_chunks.append(chunk)
        else:
            sub_chunks = _split_at_sentence_boundaries(chunk, max_tokens, 0)
