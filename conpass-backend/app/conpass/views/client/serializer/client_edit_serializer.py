...
class ClientEditGetRequestBodySerializer(serializers.Serializer):
    id = serializers.CharField()  # ID
...
class ClientEditPostRequestBodySerializer(serializers.Serializer):
    id = serializers.CharField(allow_blank=True, required=False)  # ID
    corporateName = serializers.CharField(error_messages={'blank': '会社名を入力してください。'})  # 会社名
    corporateAddress = serializers.CharField(allow_blank=True, default='')  # 住所
    corporateExecutiveName = serializers.CharField(allow_blank=True, default='')  # 代表者名
    corporateSalesName = serializers.CharField(allow_blank=True, default='')  # 営業担当者名 （担当グループ名）
...
class ClientEditResponseSerializer(serializers.Serializer):
...
class ClientEditResponseBodySerializer(serializers.Serializer):
    """
    ...
    response = ClientEditResponseSerializer()
